    ...
    async def materialize_initiatives(
        ...
        async with self._session_factory.begin() as session:
            ...
            for item in items:
                ...
                state = await self._lock_state(session, subscription, now)
    ...
    async def _lock_state(
        ...
        state = (
            await session.scalars(
                ...
                .where(InitiativeState.session_id == subscription.session_id)
                .with_for_update()
            )
        ).one()
        if state.conversation_id != subscription.conversation_id:
